...
const gridItems = Array.from({ length: 80 }, (_, i) => {
  ...
  if (id === 24) return { id, label: "Projects", image: "/homePageAssets/feature-projects.png", size: "large", color: "text-blue-500", highlight: true, useH3: true };


  if (id === 26) return { id, label: "Docs", image: "/homePageAssets/feature-docs.png", icon: FileText, size: "large", color: "text-blue-400", highlight: true, useH3: true };

  if (id === 44) return { id, label: "Brain", image: "/homePageAssets/feature-brain.png", icon: Brain, size: "large", color: "text-purple-500", highlight: true, useH3: true };

  if (id === 46) return { id, label: "Chat", image: "/homePageAssets/feature-chat.png", icon: MessageSquare, size: "large", color: "text-purple-600", highlight: true, useH3: true };
              ...
              >
                {item.image ? (
                  <Image
                    ...
                    width={500}
                    height={500}
                    className={`mb-2 w-full h-auto object-cover rounded ${isLarge ? "max-h-full" : "max-h-[100px]"}`}
                  />
                ) : item.icon && (
                  <item.icon
                ...
                {/* {!item.label && !item.icon && <span className="text-gray-400 opacity-50"></span>} */}
              </div>
            );
          })}
        </div>
